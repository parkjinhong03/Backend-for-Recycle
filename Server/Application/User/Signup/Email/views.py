...
@api.route('/User/signup/email',
           doc={'description': '사용자가 인증 번호 발급 버튼을 눌렀을 때 호출하는 APi로, '
                               '인증을 위해 email 주소를 넘겨 받아 실제로 그 email 주소로 인증 번호를 발송해주는 API (Gmail 밖에 안됨)'})
@api.doc(params={
...
@api.route('/User/signup/email/auth',
           doc={'description': '사용자가 인증 번호 발급 및 입력 후 확인 버튼을 눌렀을 때 호출하는 API로, '
                               '인증 번호를 제대로 썼다면 그 이메일에 대한 인증을 완료해주는 API 이다.'})
@api.doc(params={
    'email': 'An Email address',
